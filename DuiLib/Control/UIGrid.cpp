...
namespace DuiLib
{
	...
	int CGridUI::GetRowHeight(int row, BOOL bScaleByDPI)
	{
		...
		if(!pRowData) 
		{
			height = GetDefRowHeight();
		}
		else
		{
			if(pRowData->GetHeight() <= 0) 
				height = GetDefRowHeight();
			else 
				height = pRowData->GetHeight();
		}

		if(GetManager() && bScaleByDPI)
		{
			height = GetManager()->GetDPIObj()->ScaleInt(height);
		}
		return height;
	}
	...
	int CGridUI::GetColumnWidth(int col, BOOL bScaleByDPI)
	{
		...
		int width = 0;
		if(m_mapColWidthFixed[col] > 0) 
		{
			width = m_mapColWidthFixed[col];
		}
		else if(m_mapColumnWidth[col] > 0) 
			width = m_mapColumnWidth[col];
		else 
		{
			width = GetDefColWidth();
		}

		if(GetManager() && bScaleByDPI)
		{
			width = GetManager()->GetDPIObj()->ScaleInt(width);
		}
		return width;
	}
	...
	void CGridUI::ClearSelectedRows()
	{
	...
	void CGridUI::SelectRow(int row, BOOL bSelected, BOOL bTriggerEvent)
	{
		...
		if(IsCheckBoxSelection())
		{
			for (int col=0; col<GetColumnCount(); col++)
			{
				...
				if(GetCellType(row,col) == celltypeCheckBox)
				{
					Cell(row,col).SetCheckBoxCheck(bSelected);
				}
				return;
			}
		}
...
LABEL_END:
	...
	void CGridUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		// -1的意义是：忽略dpi缩放产生的误差。
		cxNeeded = GetManager()->GetDPIObj()->ScaleIntBack(cxNeeded) - 1;
		//cyNeeded = GetManager()->GetDPIObj()->ScaleIntBack(cyNeeded) - 1;
	...
	void CGridUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
	{
		while (m_pHorizontalScrollBar)
		{
			...
			if (cxScroll > 1 && !m_pHorizontalScrollBar->IsVisible())
			{
...
// 		}
	...
	void CGridUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("checkboxselection")) == 0 )
		{
			SetCheckBoxSelection(_tcsicmp(pstrValue, _T("true")) == 0);
		}
		//////////////////////////////////////////////////////////////////////////
